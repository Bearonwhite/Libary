...
impl PrinterShareApp {
    ...
    fn start_server_backend(&mut self) {
        ...
        // === BUG FIX: ถ้ายังไม่ได้ตั้งชื่อ printer ไม่ start server ===
        let printer_name = match &self.config.shared_printer_name {
            Some(name) if !name.trim().is_empty() => name.clone(),
            _ => {
                self.last_status =
                    "⚠️ กรุณาตั้งชื่อเครื่องปริ้นก่อนเริ่ม server".to_string();
                return;
            }
        };
        ...
        self.rt.spawn(async move {
            if let Err(e) =
                crate::server::run_server(port, printer_name, queue_dir, state, log).await
            {
...
impl PrinterShareApp {
    fn draw_setup(&mut self, ui: &mut egui::Ui) {
        ...
        ui.add_space(6.0);
        ui.small("หมายเหตุ: งานพิมพ์จริงต้องอาศัย TCP — UDP ใช้สำหรับค้นหาเครื่อง (discovery) เท่านั้น");
    }
    ...
    fn draw_server(&mut self, ui: &mut egui::Ui) {
        ...
        if ui.text_edit_singleline(&mut printer_name).changed() {
            ...
            let _ = self.config.save();
            // restart server เมื่อชื่อเปลี่ยน
            self.server_started = false;
            self.start_server_backend();
            self.mark_interaction();
        }

        // ปุ่มรายชื่อ printer (Windows เท่านั้น)
        #[cfg(windows)]
        {
            ui.label("หรือเลือกจากรายชื่อที่ติดตั้งในเครื่อง:");
        ...
        // แสดง status ว่า server พร้อมหรือยัง
        let printer_ready = self
            .config
            .shared_printer_name
            .as_ref()
        ...
        if !printer_ready {
            ui.colored_label(
                ...
                "⚠️ ยังไม่ได้ตั้งชื่อเครื่องปริ้น — กรุณาพิมพ์ชื่อหรือกดเลือกจากด้านบน",
            );
        } else if self.server_started {
            ui.colored_label(egui::Color32::from_rgb(0, 180, 0), "🟢 Server กำลังทำงาน");
        }

        ui.add_space(6.0);

        // ปุ่ม Print Test — disable ถ้ายังไม่ได้ตั้งชื่อ printer
        let btn = egui::Button::new("🖨  Print Test (พิมพ์ทดสอบจาก server เอง)");
        if ui.add_enabled(printer_ready && self.server_started, btn).clicked() {
            self.mark_interaction();
            let queue_dir = self.config.queue_dir.clone();
    ...
    fn draw_client(&mut self, ui: &mut egui::Ui) {
        ...
        ui.separator();
        ui.horizontal(|ui| {
            let btn_text = if self.scanning {
                "กำลังสแกน..."
            } else {
                "🔍 สแกนวงหาเครื่องปริ้น"
            };
            if ui
                .add_enabled(!self.scanning, egui::Button::new(btn_text))
                .clicked()
            {
                self.mark_interaction();
                self.scanning = true;
            ...
            ui.horizontal(|ui| {
                ...
                if ui
                    .selectable_label(
                        ...
                        format!(
                            "☐ {}  ({})  MAC: {} — IP: {}",
                            s.printer_name, s.hostname, s.mac_address, s.ip
                        ),
                    )
                    .clicked()
                {
